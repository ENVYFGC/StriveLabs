...
namespace {
  ...
  // Center ratios are now read from ModMenu::getLayoutPositions() at draw time.
  // These defaults are only used for the initial DrawContext constructor.
  constexpr double CENTER_X_RATIO_DEFAULT = 0.5;
  constexpr double CENTER_Y_RATIO_DEFAULT = 0.8;
...
FrameBar::Data::Data()
: tool(CENTER_X_RATIO_DEFAULT, CENTER_Y_RATIO_DEFAULT) {
  resetFrames();
}
...
void FrameBar::Data::draw() {
  // Apply user-configured anchor position before updating screen coordinates.
  // framebar_center_x/y_ratio are screen-space ratios in [0.0, 1.0].
